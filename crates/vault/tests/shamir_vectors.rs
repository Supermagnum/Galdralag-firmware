...
fn shamir_vectors_json_recover() -> Result<(), ShamirError> {
    ...
    for vec in root["vectors"].as_array().expect("vectors") {
        ...
        let secret = hex::decode(vec["secret_hex"].as_str().expect("secret_hex"))
            .expect("secret hex");
        ...
        for s in vec["shares"].as_array().expect("shares") {
            ...
            collected.push(share).map_err(|_| ShamirError::InvalidParameters)?;
        }
        let r = shamir_recover(collected.as_slice(), k)?;
        assert_eq!(r.as_slice(), secret.as_slice(), "vector {:?}", vec.get("name"));
    }
    Ok(())
}
