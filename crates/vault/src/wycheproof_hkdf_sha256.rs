//! Wycheproof JSON vectors for HKDF-SHA-256 (`tests/data/wycheproof/hkdf_sha256_test.json`).

use hkdf::Hkdf;
use sha2::Sha256;

#[test]
fn wycheproof_hkdf_sha256_json() {
    let data = include_str!("../tests/data/wycheproof/hkdf_sha256_test.json");
    let root: serde_json::Value = serde_json::from_str(data).expect("wycheproof HKDF JSON");
