//! Wycheproof JSON vectors for HMAC-SHA-512 (`tests/data/wycheproof/hmac_sha512_test.json`).
...
#[test]
fn wycheproof_hmac_sha512_json() {
    let data = include_str!("../tests/data/wycheproof/hmac_sha512_test.json");
    let root: serde_json::Value = serde_json::from_str(data).expect("wycheproof HMAC-SHA512 JSON");
    ...
    for group in groups {
        ...
        for t in tests {
            ...
            let mut mac = <HmacSha512 as hmac::digest::KeyInit>::new_from_slice(&key).expect("hmac key");
            mac.update(&msg);
