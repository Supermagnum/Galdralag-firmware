//! Wycheproof JSON vectors for Brainpool P-512r1 ECDH and ECDSA-SHA512 (`tests/data/`).

use crate::brainpool512::{
...
/// Wycheproof encodes private scalars as minimal hex or with leading zero bytes; normalize to 64-byte
/// big-endian field elements (strip leading zeros, then left-pad).
fn decode_p512_private_scalar(tc_id: u64, private_hex: &str) -> Vec<u8> {
    ...
    if v.len() > 64 {
        panic!("tcId {tc_id} private scalar longer than 64 bytes after normalization");
    }
    if v.len() < 64 {
        let mut padded = vec![0u8; 64];
        padded[64 - v.len()..].copy_from_slice(&v);
        v = padded;
    }
    v
}
...
fn run_ecdh_case(tc_id: u64, public_hex: &str, private_hex: &str, shared_hex: &str, result: &str) {
    let private_bytes = decode_p512_private_scalar(tc_id, private_hex);
    ...
    match result {
        "valid" => {
            let pk = BrainpoolP512PublicKey::from_public_key_der(&pub_der)
                .unwrap_or_else(|e| panic!("tcId {tc_id} valid public DER: {e:?}"));
            let scalar = BrainpoolP512Scalar::from_secret_key_bytes_for_test(&private_bytes)
        ...
        "acceptable" => {
            let Ok(pk) = BrainpoolP512PublicKey::from_public_key_der(&pub_der) else {
                return;
            };
            let Ok(scalar) = BrainpoolP512Scalar::from_secret_key_bytes_for_test(&private_bytes)
            else {
                return;
            };
        ...
        "invalid" => {
            let sk_ok = SecretKey::<BrainpoolP512r1>::from_slice(&private_bytes).ok();
            let pk_ok = BrainpoolP512PublicKey::from_public_key_der(&pub_der).ok();
            match (sk_ok, pk_ok) {
                (Some(_), Some(pk)) => {
                    let Ok(scalar) =
                        BrainpoolP512Scalar::from_secret_key_bytes_for_test(&private_bytes)
                    else {
                        return;
                    };
...
#[test]
fn wycheproof_brainpool512_ecdh_json() {
    let data = include_str!("../tests/data/ecdh_brainpoolP512r1_test.json");
...
fn run_ecdsa_case(
    tc_id: u64,
    vk: &BrainpoolP512VerifyingKey,
    msg: &[u8],
    sig_raw: &[u8],
    result: &str,
) {
    let sig = match BrainpoolP512Signature::from_der_bytes_for_test(sig_raw) {
        Ok(s) => s,
        Err(_) => {
            assert_eq!(
                result, "invalid",
...
#[test]
fn wycheproof_brainpool512_ecdsa_sha512_json() {
    let data = include_str!("../tests/data/ecdsa_brainpoolP512r1_sha512_test.json");
    let root: serde_json::Value =
    ...
    for group in groups {
        ...
        let vk = BrainpoolP512VerifyingKey::from_public_key_der(&der_bytes)
