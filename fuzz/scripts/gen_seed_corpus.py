...
def builtin_profiles() -> dict[str, bytes]:
    ...
    return {
        ...
        "high-assurance": profile_to_bytes(
            "high-assurance",
            "Serpent, Twofish, ChaCha; BP512r1; Shamir 3-of-5",
            CURVE_BP512,
            [LAYER_SERPENT, LAYER_TWOFISH, LAYER_CHACHA],
            3,
            5,
        ),
    }
...
# brainpool512.rs test G_SEC1 (129 bytes)
G512_SEC1 = bytes(
    [
        0x04,
        0x81,
...
def main() -> None:
    ...
    write_bytes(seed_root / "brainpool512_ecdh" / "generator_sec1_uncompressed.bin", G512_SEC1)
    # Extra malformed / edge SEC1 blobs
    write_bytes(seed_root / "brainpool384_ecdh" / "truncated_generator.bin", G384_SEC1[:48])
    write_bytes(seed_root / "brainpool512_ecdh" / "truncated_generator.bin", G512_SEC1[:64])
