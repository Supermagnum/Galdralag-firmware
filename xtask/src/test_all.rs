...
pub fn run(workspace_root: &Path, skip_fuzz: bool) -> i32 {
    ...
    eprintln!("test-all: 12/14 timing-test (security-tests)");
    log.push_step(
        "timing-test",
        cargo_ok(workspace_root, &["test", "-p", "security-tests", "-q"]),
    );
    ...
    if !fuzz_ok && !fuzz_skipped {
        eprintln!("test-all: WARNING fuzz step had issues; see Section 9 and 12 in TEST_RESULTS.md");
    }
    0
}
...
fn build_markdown(
    ...
    s.push_str("## 2. Wycheproof vector results\n\n");
    ...
    s.push_str("## 3. RFC test vector results\n\n");
    ...
    s.push_str("## 4. BSI test vector results\n\n");
    ...
    s.push_str("## 5. NIST CAVP vector results\n\n");
    ...
    s.push_str("## 6. Known-answer test (KAT) results\n\n");
    ...
    s.push_str("## 7. Key lifecycle tests\n\n");
    s.push_str("Integration tests in `vault/tests/key_lifecycle.rs`. ");
    s.push_str("Last run: **");
    s.push_str(step_status(steps, "key_lifecycle"));
    s.push_str("**.\n\n");

    s.push_str("## 8. dudect timing results\n\n");
    s.push_str("| Harness | Samples | t-statistic | Threshold | Result |\n");
    s.push_str("|---------|---------|-------------|-----------|--------|\n");
    s.push_str("| (security-tests stubs) | 0 | N/A | n/a | **NotRun** (stubs only; wire dudect for production metrics) |\n\n");

    s.push_str("## 9. cargo-fuzz coverage summary\n\n");
    if fuzz_skipped {
    ...
    s.push_str("## 10. Zeroisation tests (simulation)\n\n");
    ...
    if fuzz_skipped {
        ...
        if !fuzz_ok {
            s.push_str("- **cargo-fuzz:** See Section 9. Install `cargo-fuzz`, use nightly if required, or run fuzz targets manually for longer sessions.\n");
        }
        if u_fail > 0 {
