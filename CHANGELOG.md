...
- **RustCrypto / dalek pins:** `hkdf` 0.13, `hmac` 0.13, `sha1`/`sha2` 0.11, `pbkdf2` 0.13, `ff` 0.14, `vsss-rs` 6.0, and `x25519-dalek` 3.0. X25519 keygen fills 32 TRNG bytes then `StaticSecret::from` (avoids `rand_core` 0.10 `CryptoRng`). `precomputed-tables` stays off (`default-features = false`).
