...
- **RustCrypto / dalek pins:** `hkdf` 0.13, `hmac` 0.13, `sha1`/`sha2`/`sha3` 0.11, `pbkdf2` 0.13, `ff` 0.14, `vsss-rs` 6.0, `x25519-dalek` 3.0, and the AEAD stack `aead` 0.6 with `aes-gcm` / `chacha20poly1305` 0.11 (`AeadInOut`, hybrid-array keys/nonces). X25519 keygen fills 32 TRNG bytes then `StaticSecret::from` (avoids `rand_core` 0.10 `CryptoRng`). `precomputed-tables` stays off (`default-features = false`). Cascade-critical checks (RFC 8439, CESS cascade KAT, `cascade_encrypt`/`cascade_decrypt` integration, dudect `timing_chacha_tag_check` / `timing_aes_gcm_tag_check`) were re-run under the new AEAD majors.
- **Host HTTP stack:** `axum` 0.8 with `utoipa-swagger-ui` 9; `galdrad` routes use `{param}` path syntax. Also `thiserror` 2, `dirs` 6, `bitflags` 2.13, and root-lockfile `rkyv` 0.8.18 (does **not** clear the firmware `services/galdralag` `rkyv` advisory — see [THREAT_MODEL.md](docs/THREAT_MODEL.md)).
- **Dependabot:** stop scanning the nested `fuzz/` Cargo workspace. Path-deps that use `foo.workspace = true` caused Dependabot “in /fuzz” PRs to rewrite root workspace pins and break firmware CI. Refresh `fuzz/Cargo.lock` in the same PR as the root bump (`.github/dependabot.yml`).
