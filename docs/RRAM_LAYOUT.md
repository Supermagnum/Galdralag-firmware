...
| **USB CCID OpenPGP (`baochip-openpgp`, Xous)** | **DO store** at **67,072**; PIN verifier digests + monotonic counters; **CCID master record** at **75,376** (36 B: `OGMK` + salt); **PIN provision slots** at **75,412** and **75,449** (37 B each: `PNU1` / `PNA1` + 1-byte length + up to **32** B PIN payload); exclusive end **75,486** | OpenPGP allows PW1/PW3 up to **127** bytes each; this firmware caps persisted provision PINs at **32** bytes (`CCID_PIN_PROVISION_PAYLOAD_MAX_BYTES`). **First-boot operator PINs** are written here by **`write_provisioning_pins`** (USB CDC provisioning) or dev/TRNG fallback paths, then read for `OpenPgpVaultBackend::new` and **cleared** afterward. Longer PINs are rejected (`HalError::Denied`), not truncated. Physical base = **`vault_phys_base()`** (`BOOT1_START - HW_RERAM_MEM`). **Not** cross-checked in Rust against [Baochip-1x-firmware](https://github.com/Supermagnum/Baochip-1x-firmware); see [Platform reconciliation](#platform-reconciliation). |
...
| **Contact store** (`contact-store`) | Base **76,800** (`0x12C00`); RRAM band ends **238,080**; layout end **238,116** (marker + integrity mirror) | **54** x **256** B records (**13,824** B); **64 KiB** string heap; **64 KiB** key-material region; **16 KiB** append-only timing log; **4** B provision marker; **32** B integrity digest mirror. Always-on monotonic counter slot **7** (`CONTACT_STORE_PROVISION_COUNTER`) incremented once on `provision_fresh`. Constants in [`crates/contact-store/src/layout.rs`](../crates/contact-store/src/layout.rs). |
