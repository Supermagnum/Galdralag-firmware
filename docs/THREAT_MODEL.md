...
| **T11** | Theft of fewer than **K** Shamir shares | Cannot reconstruct secret (Shamir information-theoretic threshold) | Mathematical threshold property **when split uses unpredictable polynomial coefficients** (`ShamirSplitRng` / OS or hardware TRNG on host; future on-device split must use platform TRNG) | **Historical violation (fixed `7db5e08851b2f0c48b65a00caa579f1d5ec077dd`):** host split before that commit used a fixed-seed `FakeTrng`; one share plus source recovered the secret — see [SECURITY_ADVISORY_SHAMIR_RNG.md](SECURITY_ADVISORY_SHAMIR_RNG.md). Poor share storage (all shares on one laptop) collapses to a single point of failure operationally. |
...
| **T15** | Network attacker with precise timing of RSA PKCS#1 v1.5 decryption (**Marvin**, RUSTSEC-2023-0071 / CVE-2023-49092) against pinned `rsa` 0.9.10 | Residual exposure is the vulnerable crate linked for OAEP/PSS helpers and PKCS#1 sign — not “any correspondent can Marvin-attack this token’s RSA DEC slot via a PGP message.” | Galdralag does not implement PKCS#1 v1.5 decrypt: vault `decrypt_oaep` uses OAEP; OpenPGP card DEC/SIG/AUT never call `rsa`. PKCS#1 v1.5 appears only as `sign_pkcs1_sha256` / `verify_pkcs1_sha256` (marker `Pkcs1v15`), unused by APDU dispatch. Dudect RSA harnesses do not measure PKCS#1 v1.5 decrypt. No patched **0.9.x**; **0.10** is a breaking upgrade not yet evaluated. | **Accepted known limitation** until `rsa` 0.10 (or a replacement) is evaluated. Still reported by `cargo audit`. See [Tracked dependency advisories](#tracked-dependency-advisories-ci-cargo-audit-allow-list). |

**T15 description (accepted-risk status unchanged):** RUSTSEC-2023-0071 applies to the pinned `rsa` 0.9.10 crate. Marvin targets PKCS#1 v1.5 decryption. Galdralag does not implement PKCS#1 v1.5 decrypt: vault `decrypt_oaep` uses OAEP; OpenPGP card DEC/SIG/AUT never call `rsa`. PKCS#1 v1.5 appears only as `sign_pkcs1_sha256` / `verify_pkcs1_sha256` (marker `Pkcs1v15`), unused by APDU dispatch. Residual exposure is "vulnerable crate linked for OAEP/PSS helpers and PKCS#1 sign," not "any correspondent can Marvin-attack this token's RSA DEC slot via a PGP message." Dudect RSA harnesses do not measure PKCS#1 v1.5 decrypt. Status remains accepted until `rsa` 0.10 (or a replacement) is evaluated.
...
- **Marvin timing** in the pinned `rsa` 0.9.10 crate (**T15**) until a breaking 0.10 migration lands. PKCS#1 v1.5 decrypt is not implemented on this token; see T15.
