...
| **T11** | Theft of fewer than **K** Shamir shares | Cannot reconstruct secret (Shamir information-theoretic threshold) | Mathematical threshold property **when split uses unpredictable polynomial coefficients** (`ShamirSplitRng` / OS or hardware TRNG on host; future on-device split must use platform TRNG) | **Historical violation (fixed `d8628017dfd07afd352e7384b53f9e06b80ce41a`):** host split before that commit used a fixed-seed `FakeTrng`; one share plus source recovered the secret — see [SECURITY_ADVISORY_SHAMIR_RNG.md](SECURITY_ADVISORY_SHAMIR_RNG.md). Poor share storage (all shares on one laptop) collapses to a single point of failure operationally. |
