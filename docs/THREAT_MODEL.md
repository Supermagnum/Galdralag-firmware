...
**Related reading:** [README.md](../README.md) (security posture, PIN policy, zeroisation), [BIOMETRIC_API.md](BIOMETRIC_API.md), [OPENPGP_CARD.md](OPENPGP_CARD.md), [CIPHER_PROFILES.md](CIPHER_PROFILES.md), [CIPHER_PROFILE_SECURITY.md](CIPHER_PROFILE_SECURITY.md), [PQ_SIGNATURES.md](PQ_SIGNATURES.md), [HARDWARE_VERIFICATION.md](HARDWARE_VERIFICATION.md), [AUDIT_LOG.md](AUDIT_LOG.md), [THREE_FACTOR_AUTH.md](THREE_FACTOR_AUTH.md).
...
|----|----------------------|---------|------------|--------|
| **T1** | Stolen token; PIN unknown | Cannot complete PIN verification; protected operations remain blocked | On-device PIN policy and counters; PIN comparison uses `ConstantTimeEq` in `pin-policy` | **Hardware** timing or fault attacks against PIN verification **not** characterised on silicon (Q2). Dudect runs on **host** harnesses for selected primitives — **not** a complete proof for the full PIN path on chip. |
...
| **T8** | Physical attack (glitching, **power analysis**, debug/JTAG) | Potential extraction or misuse of secrets | Xous separation, sealed vault, zeroisation policies ([HARDWARE_VERIFICATION.md](HARDWARE_VERIFICATION.md)) | **No** independent hardware security evaluation reported. Open RTL aids **review** and aids **attackers** equally. |
...
- **Physical attacks** without independent silicon evaluation (**T8**).
