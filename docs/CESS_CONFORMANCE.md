...
**Built-in mapping:** `standard` → **`0x0001`**; `conservative` and `conservative-shamir` → **`0x0003`** (cascade ChaCha inner, Serpent outer on **BrainpoolP256r1** session curve in Galdralag; CESS table rows use **P384** for the inner KEM column — see deviation register). Built-in cascade **layer order** matches the registry text (ChaCha then Serpent in the profile list = inner then outer). The former **`high-assurance`** profile mapped to **`0x0012`** (P-512 cascade) and has been **removed**; see [CHANGELOG.md](../CHANGELOG.md). **Migrating** from prior private-use `suite_id` values (`0xE001`–`0xE004`), from **`0x0012` / `high-assurance`**, or from the old **Serpent-then-ChaCha** order requires re-encrypting; old ciphertexts are not readable with the new defaults.
