...
- **`test-all --no-dudect`:** Skips the **dudect** timing suite (~15–20 minutes). CI runs dudect; use this flag only for quick local runs. Prefer `cargo run -p xtask -- timing-test` when iterating on a single harness.
