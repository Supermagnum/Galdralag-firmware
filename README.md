...
- **Unlock full-disk encryption** with **Linux Unified Key Setup (LUKS)** by protecting the volume passphrase or keyfile with an OpenPGP key held on a smart card.

**GnuPG and LUKS.** **LUKS** is the standard on-disk format for block-device encryption on Linux (managed with **`cryptsetup`**). LUKS itself does not perform public-key cryptography; instead it unlocks a volume from one of several **key slots**, each holding a passphrase or keyfile. GnuPG can sit in front of that: a random **keyfile** is generated, encrypted to your OpenPGP key with **`gpg --encrypt`** (so the plaintext keyfile never rests on disk), and at boot or mount time **`gpg --decrypt`** releases it to `cryptsetup` to open the volume. When the OpenPGP key lives on a hardware token such as this one, the LUKS keyfile can only be decrypted while the token is present and the **PIN** has been entered, so the disk stays sealed if the token is removed. This is what lets a smart card gate access to an encrypted root or data partition. The keyfile decryption is an ordinary host-side **`gpg`** operation; LUKS slot management, boot integration (for example `crypttab` or initramfs hooks), and recovery key handling are configured with `cryptsetup` and your distribution's tooling, not by this firmware. For splitting the unlock secret across multiple holders, see [Shamir secret sharing and drive encryption](#shamir-secret-sharing-and-drive-encryption).
